...
class door_control:
    def __init__(self):
        ...
        self.label_map = {"thumb": 0, "palm_moved": 1}
    ...
    def extract_and_load_dataset(self, zip_path, extract_dir="dataset"):
        ...
        for person_dir in os.listdir(dataset_root):
            ...
            for gesture_dir in os.listdir(person_path):
                ...
                if gesture_dir.lower() == "05_thumb":
                    label = "thumb"
                elif gesture_dir.lower() == "08_palm_moved":
                    label = "palm_moved"
                else:
                    continue
    ...
    def detect_gestures(self):
        ...
        door_open = most_common == "thumb"
        door_close = most_common == "palm_moved"

        if door_open:
            cv2.putText(img, "Kapı Açılıyor!", (10, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
            cv2.rectangle(img, (0, 0), (w, h), (0, 255, 0), 2)
        elif door_close:
            cv2.putText(img, "Kapı Kapanıyor!", (10, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
            cv2.rectangle(img, (0, 0), (w, h), (0, 0, 255), 2)

        return door_open, door_close, img
