...
class curtain_control:
    def __init__(self):
        ...
        self.label_map = {"palm": 0, "fist_moved": 1}
    ...
    def extract_and_load_dataset(self, zip_path, extract_dir="dataset"):
        ...
        for person_dir in os.listdir(dataset_root):
            ...
            for gesture_dir in os.listdir(person_path):
                ...
                if gesture_dir.lower() == "01_palm":
                    label = "palm"
                elif gesture_dir.lower() == "04_fist_moved":
                    label = "fist_moved"
                else:
                    continue
    ...
    def detect_gestures(self):
        ...
        is_open = most_common == "palm"
        is_close = most_common == "fist_moved"

        if is_open:
            cv2.putText(img, "Perde Aciliyor!", (10, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
            cv2.rectangle(img, (0, 0), (w, h), (0, 255, 0), 2)
        elif is_close:
            cv2.putText(img, "Perde Kapaniyor!", (10, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
            cv2.rectangle(img, (0, 0), (w, h), (0, 0, 255), 2)

        return is_open, is_close, img
